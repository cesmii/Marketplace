namespace CESMII.Marketplace.Api.Shared.Utils
{
    ...
    public class TokenUtils
    {
        ...
        public TokenModel BuildToken(UserModel user)
        {
            ...
            return new TokenModel { UserID = user.ID, UserName = user.UserName, Token = tokenHandler.WriteToken(newToken), ExpirationDate = expirationDate, IsImpersonating = false };
        }
        ...
        public TokenModel BuildImpersonationToken(UserModel user, string targetUserID)
        {
            var expirationDate = DateTime.UtcNow.AddMinutes(_config.DefaultExpiration);
            var tokenDescriptor = BuildTokenDescriptor(user, expirationDate);
            ...
            var tokenHandler = new JwtSecurityTokenHandler();
            var newToken = tokenHandler.CreateToken(tokenDescriptor);
            return new TokenModel { UserID = user.ID, UserName = user.UserName, Token = tokenHandler.WriteToken(newToken), ExpirationDate = expirationDate, IsImpersonating = true };
        }
