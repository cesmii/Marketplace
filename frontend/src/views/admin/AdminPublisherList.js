...
import AdminPublisherRow from './shared/AdminPublisherRow';
import { getPublisherPreferences, setPublisherPreferencesPageSize } from '../../services/PublisherService';

const CLASS_NAME = "AdminPublisherList";

function AdminPublisherList() {
    ...
    const _publisherPreferences = getPublisherPreferences();
    const [_pager, setPager] = useState({ currentPage: 1, pageSize: _publisherPreferences.pageSize, searchVal: null });
    ...
    const onChangePage = (currentPage, pageSize) => {
        ...
        setPublisherPreferencesPageSize(pageSize);
    };
    ...
    const renderItemsGridHeader = () => {
        ...
        return (
            <thead>
                <AdminPublisherRow key="header" item={null} isHeader={true} cssClass="admin-item-row" />
            </thead>
        )
    }
    ...
    const renderItemsGrid = () => {
        ...
        const mainBody = _dataRows.all.map((item) => {
            return (
                <AdminPublisherRow key={item.id} item={item} cssClass={`admin-item-row`} onDeleteItem = { onDeleteItem } />
            );
        });
                ...
                <div className="col-sm-12 d-flex align-items-center" >
                    {(_dataRows.itemCount != null && _dataRows.itemCount > 0) ?
                        <>
                            ...
                            <a className="btn btn-icon-outline circle primary" href={`/admin/publisher/new`} ><i className="material-icons">add</i></a>
                        </>
                        :
                        <a className="btn btn-icon-outline circle ml-auto primary" href={`/admin/publisher/new`} ><i className="material-icons">add</i></a>
                    }
                </div>
            </div>
