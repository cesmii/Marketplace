...
import AdminMarketplaceRow from './shared/AdminMarketplaceRow';
import { clearSearchCriteria, getMarketplacePreferences, setMarketplacePageSize } from '../../services/MarketplaceService';

const CLASS_NAME = "AdminMarketplaceList";

function AdminMarketplaceList() {
    ...
    const renderItemsGridHeader = () => {
        ...
        return (
            <thead>
                <AdminMarketplaceRow key="header" item={null} isHeader={true} cssClass="admin-item-row" />
            </thead>
        )
    }
    ...
    const renderItemsGrid = () => {
        ...
        const mainBody = _dataRows.all.map((item) => {
            return (
                <AdminMarketplaceRow key={item.id} item={item} cssClass={`admin-item-row`} onDeleteItem = { onDeleteItem } />
            );
        });
                ...
                <div className="col-sm-12 d-flex align-items-center" >
                    {(_dataRows.itemCount != null && _dataRows.itemCount > 0) ?
                        <>
                            ...
                            <a className="btn btn-icon-outline circle primary" href={`/admin/library/new`} ><i className="material-icons">add</i></a>
                        </>
                        :
                        <a className="btn btn-icon-outline circle ml-auto primary" href={`/admin/library/new`} ><i className="material-icons">add</i></a>
                    }
                </div>
            </div>
