...
const CLASS_NAME = "AdminEmailList";

function AdminEmailList(props) {
    ...
    const renderNoDataRow = () => {
        return (
            <div className="alert alert-info-custom mt-2 mb-2">
                <div className="text-center" >There are no emails.</div>
            </div>
        );
    }
    ...
    const renderItemsGridHeader = () => {
        ...
        return (
            <AdminEmailRow key="header" item={null} isHeader={true} cssClass={`admin-item-row`} />
        )
    }
    ...
    const renderItemsGrid = () => {
        ...
        return props.items.map((item, counter) => {
            ...
            return (
                <AdminEmailRow key={key} item={item} cssClass={`admin-item-row`}
                    type={props.type} onChangeItem={props.onChangeItem} onDeleteEmail={props.onDeleteEmail} />
            );
        });
    }
