...
import { getJobDefinitionPreferences, setJobDefinitionPreferencesPageSize } from '../../services/JobDefinitionService';
import AdminJobDefinitionRow from './shared/AdminJobDefinitionRow';
import ConfirmationModal from '../../components/ConfirmationModal';
import color from '../../components/Constants';

const CLASS_NAME = "AdminJobDefinitionList";

function AdminJobDefinitionList() {
    ...
    const _preferences = getJobDefinitionPreferences();
    ...
    const onChangePage = (currentPage, pageSize) => {
        ...
        setJobDefinitionPreferencesPageSize(pageSize);
    };
    ...
    useEffect(() => {
        async function fetchData() {
            ...
            var url = `admin/jobDefinition/search`;
    ...
    const renderItemsGridHeader = () => {
        ...
        return (
            <thead>
                <AdminJobDefinitionRow key="header" item={null} isHeader={true} cssClass="admin-item-row" />
            </thead>
        )
    }
    ...
    const renderItemsGrid = () => {
        ...
        const mainBody = _dataRows.all.map((item) => {
            return (
                <AdminJobDefinitionRow key={item.id} item={item} cssClass={`admin-item-row`} onDeleteItem = { onDeleteItem } />
            );
        });
                ...
                <div className="col-sm-12 d-flex align-items-center" >
                    {(_dataRows.itemCount != null && _dataRows.itemCount > 0) ?
                        <>
                            ...
                            <a className="btn btn-icon-outline circle primary" href={`/admin/jobDefinition/new`} ><i className="material-icons">add</i></a>
                        </>
                        :
                        <a className="btn btn-icon-outline circle ml-auto hl-blue" href={`/admin/jobDefinition/new`} ><i className="material-icons">add</i></a>
                    }
                </div>
            </div>
...
export default AdminJobDefinitionList;