...
import AdminLookupRow from './shared/AdminLookupRow';
import ConfirmationModal from '../../components/ConfirmationModal';
import color from '../../components/Constants';

const CLASS_NAME = "AdminLookupList";

function AdminLookupList() {
    ...
    const _lookupPreferences = getLookupPreferences();
    const [_pager, setPager] = useState({ currentPage: 1, pageSize: _lookupPreferences.pageSize, searchVal: null });
    ...
    const onChangePage = (currentPage, pageSize) => {
        ...
        setLookupPreferencesPageSize(pageSize);
    };
    ...
    const renderNoDataRow = () => {
        return (
            <div className="alert alert-info-custom mt-2 mb-2">
                <div className="text-center" >There are no matching lookup items.</div>
            </div>
        );
    }
    ...
    const renderItemsGridHeader = () => {
        ...
        return (
            <thead>
                <AdminLookupRow key="header" item={null} isHeader={true} cssClass="admin-item-row" />
            </thead>
        )
    }
    ...
    const renderItemsGrid = () => {
        ...
        const mainBody = _dataRows.all.map((item) => {
            toggleShaded = grpName !== item.lookupType.name ? !toggleShaded : toggleShaded;
            var shading = toggleShaded ? "bg-light" : "";
            grpName = item.lookupType.name;
            return (
                <AdminLookupRow key={item.id} item={item} cssClass={`admin-item-row ${shading}`} onDeleteItem = { onDeleteItem } />
            );
        });
...
export default AdminLookupList;