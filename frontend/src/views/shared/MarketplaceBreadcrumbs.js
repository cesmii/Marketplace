...
function MarketplaceBreadcrumbs(props) {
    ...
    const result = _items.map((p, i) => {
        var delimiter = i < _items.length - 1 ? (<span className="me-2" >/</span>) : "";
        return (
            <Fragment key={`crumb_${p.id}`} >
                {i < _items.length - 1 ? renderLinkedName(p, 'me-2') : p.name }
                { delimiter }
            </Fragment>
        );
    });
    ...
    return (
        <div key="breadcrumbs" className="row breadcrumbs m-0">
            <div className="col-sm-12 m-0 p-0">
                <span className="me-2" ><SVGIcon name="schema" size="18" fill={color.shark} alt="breadcrumbs" /></span>
                {result}
            </div>
        </div>
    );
